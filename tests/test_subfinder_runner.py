...
def test_normalize_host_handles_schemeless_urls_and_credentials():
    assert _normalize_host("example.com/path?q=1") == "example.com"
    assert _normalize_host("user:pass@api.example.com:8443/admin") == "api.example.com"
    assert _normalize_host("www.example.org#section") == "www.example.org"


def test_extract_project_root_domains_from_url_heavy_project_list():
    hosts = [
        "example.com/login https://shop.example.com/cart",
        "user:pass@admin.example.net:8443/dashboard",
        "https://deep.api.demo.co.uk/v1?x=1",
    ]

    assert _extract_project_root_domains(hosts) == ["demo.co.uk", "example.com", "example.net"]
...
def test_projects_with_root_domains_extracts_roots_from_project_host_lists(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda pid: ["https://api.example.com", "www.example.org/path"] if pid == "p1" else [],
    )
